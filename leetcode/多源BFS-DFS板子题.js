...
var DetectSquares = function() {
    // 思路: 嵌套哈希记录(x, (y, count))，先找到x坐标相等的点,左右计算即可, 赋值不能加else。。。
    // 注意哈希需要先new再set，不能在new的时候塞值
    // 关键代码: if (!row) { return 0;
    // 关键代码: if (y2 === y) { continue
    // 关键代码: ans += row.get(y2) * row2.get(y) * row2.get(y2);
    // 关键代码: ans += row.get(y2) * row3.get(y) * row3.get(y2);
...
DetectSquares.prototype.count = function(point) {
    ...
    let ans = 0;
    for (const [y2, count] of row) { // y2 是 x坐标相同的竖边上的 y点, count是[x, y2]点的个数
        if (y2 === y) { // 点在point上是本身不符合
            continue;
        }
        const d = Math.abs(y2 - y); // 算出边长
        // count 是x坐标相同的上下的点的数量就是row.get(y2);
        // row2 是左边 row2.get(y)是左1  row2.get(y2)是左下;
        // row3 是右边 row3.get(y)是右1  row3.get(y2)是右下;
        const row2 = this.map.get(x - d); // 左边
        if (row2 && row2.has(y) && row2.has(y2)) {
            ans += count * row2.get(y) * row2.get(y2);
        }
        const row3 = this.map.get(x + d); // 右边
        if (row3 && row3.has(y) && row3.has(y2)) {
            ans += count * row3.get(y) * row3.get(y2);
        }
    }
    return ans;
};
